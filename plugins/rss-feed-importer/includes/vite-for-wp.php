...
namespace WatchTheDot\Plugins\RSSImporter\ThirdParty\Vite;
...
EOS;
	...
	add_filter(
		...
		static function ( array $attributes ) use ( $script_position ): array {
...
function load_production_asset( object $manifest, string $entry, array $options ): ?array {
	...
	if ( ! empty( $item->imports ) ) {
		...
		$check_imports = static function ( array $imports ) use ( &$check_imports, &$assets, $manifest, $url, $options ): void {
