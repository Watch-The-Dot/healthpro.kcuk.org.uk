...
$button_class = 'et_pb_button et_pb_bg_layout_light';
...
<div class="ld-content-actions">
	...
	?>
	<div class="ld-content-action<?php echo ! $learndash_previous_step_id ? ' ld-empty' : ''; ?> learndash--navigation--prev-wrapper">
		<?php if ( $learndash_previous_step_id ) : ?>
			<a class="<?php echo esc_attr( $button_class ); ?> learndash--navigation--prev" href="<?php echo esc_url( learndash_get_step_permalink( $learndash_previous_step_id, $course_id ) ); ?>">
				<span class="ld-icon <?php echo $rtl( 'ld-icon-arrow-right', 'ld-icon-arrow-left' ); ?>"></span>
	...
	<?php if ( $parent_id && 'focus' !== $context ) : ?>
		<?php if ( $learndash_maybe_show_next_step_link ) : ?>
			<div class="ld-content-action learndash--navigation--step-back-wrapper">
				<?php if ( ( true === $can_complete ) && ( true !== $current_complete ) && ( ! empty( $complete_button ) ) ) : ?>
					<?php echo learndash_mark_complete( $course_step_post ); ?>
				<?php elseif ( ( true === $can_complete ) && ( true === $current_complete ) && ( ! empty( $incomplete_button ) ) ) : ?>
					<?php echo $incomplete_button; ?>
				<?php endif; ?>
				<a	href="<?php echo esc_url( learndash_get_step_permalink( $parent_id, $course_id ) ); ?>" 
					class="et_pb_button et_pb_bg_layout_light learndash--navigation--step-back">
					<?php echo learndash_get_label_course_step_back( get_post_type( $parent_id ) ); ?>
				</a>
			</div>
			<div class="ld-content-action<?php echo ! $learndash_next_step_id ? ' ld-empty' : ''; ?> learndash--navigation--next-wrapper">
				<?php if ( $learndash_next_step_id ) : ?>
					<a class="<?php echo esc_attr( $button_class ); ?> learndash--navigation--next" href="<?php echo esc_url( learndash_get_step_permalink( $learndash_next_step_id, $course_id ) ); ?>">
						<span class="ld-text"><?php echo learndash_get_label_course_step_next( get_post_type( $learndash_next_step_id ) ); ?></span>
						<span class="ld-icon <?php echo $rtl( 'ld-icon-arrow-left', 'ld-icon-arrow-right' ); ?>"></span>
					</a>
				<?php endif; ?>
			</div>
		<?php else : ?>
			...
			<div class="ld-content-action<?php echo ( ! $can_complete ) && ( ! $learndash_next_step_id ) ? ' ld-empty' : ''; ?>">
				<?php if ( ( true === $can_complete ) && ( true !== $current_complete ) && ( ! empty( $complete_button ) ) ) : ?>
					<?php echo $complete_button; ?>
				<?php elseif ( $learndash_next_step_id ) : ?>
					<a class="<?php echo esc_attr( $button_class ); ?> learndash--navigation--next" href="<?php echo esc_attr( learndash_get_step_permalink( $learndash_next_step_id, $course_id ) ); ?>">
						<span class="ld-text"><?php echo learndash_get_label_course_step_next( get_post_type( $learndash_next_step_id ) ); ?></span>
						<span class="ld-icon <?php echo $rtl( 'ld-icon-arrow-left', 'ld-icon-arrow-right' ); ?>"></span>
					</a>
				<?php endif; ?>
